...
# decooder network
from Multimodal_Contrast.networks.decoding_network import ContrastiveM3LDecoderNetwork
...
class MultimodalContrastiveTM:
    ...
    def __init__(self, bow_size, contextual_sizes=(768, 2048), n_components=10, model_type='prodLDA',
        ...
        #### OLD langs is an array of language codes e.g. ['en', 'de']
        #langs is an array of language code. 
        
        self.languages = languages
        self.num_lang = 1 #len(languages)
    ...
    def _train_epoch(self, loader):
        ...
        for batch_samples in loader:
            ...
            #posterior_log_variance, word_dists, estimated_labels, img_feature_dists = self.model(X_bow, X_contextual, labels, X_image_embeddings =  X_image_embeddings)
            ...
            kl_en_prior = self._kl_loss2(prior_mean, prior_variance,  
                                        posterior_mean1, posterior_variance1, posterior_log_variance1)
            #kl_de_prior = self._kl_loss2(prior_mean, prior_variance,
                                        #posterior_mean2, posterior_variance2, posterior_log_variance2)
            kl_image_prior = self._kl_loss2(prior_mean, prior_variance,
                                           posterior_mean3, posterior_variance3, posterior_log_variance3)

            # KL loss between posterior distributions of paired languages/modalities
            #Interesting they have three KL divergence in mutlimodal
            ...
            loss = rl_loss1 +  \
                   + self.weights["KL"] * kl_en_prior + self.weights["KL"] * kl_image_prior \
        ...
        for sample_index in range(n_samples):
            with torch.no_grad():
                ...
                for batch_samples in loader:
                    ...
                    thetas = self.model.get_theta(x=X_bow, x_bert=X_contextual, lang_index=lang_index, x_image=X_image)
    ...
    def get_wordcloud(self, topic_id, n_words=5, background_color="black", width=1000, height=400): #havent been tested
        """
    ...
    def get_word_distribution_by_topic_id_crosslingual(self, topic, language_idx=0): #havent been tested
        """
        Return the word probability distribution of a topic sorted by probability.
        ...
        if topic >= self.n_components:
            ...
            wd = self.get_topic_word_distribution()
            t = [(word, wd[topic][idx]) for idx, word in self.train_data.idx2token[language_idx].items()]
            t = sorted(t, key=lambda x: -x[1])
        return t

    def get_wordcloud_crosslingual(self, topic_id, n_words=5, background_color="black", width=1000, height=400, language_idx=0):
        """
        Plotting the wordcloud. It is an adapted version of the code found here:
        http://amueller.github.io/word_cloud/auto_examples/simple.html#sphx-glr-auto-examples-simple-py and
        here https://github.com/ddangelov/Top2Vec/blob/master/top2vec/Top2Vec.py
        ...
        :param height: height of the produced image
        """
        #word_score_list = self.get_word_distribution_by_topic_id_crosslingual(topic_id,language_idx)[:n_words]
        word_score_list = get_word_distribution_by_topic_id_crosslingual(self,topic_id,language_idx)[:n_words]
        word_score_dict = {tup[0]: tup[1] for tup in word_score_list}
        plt.figure(figsize=(10, 4), dpi=200)
        plt.axis("off")
        plt.imshow(wordcloud.WordCloud(width=width, height=height, background_color=background_color
    ...
    def get_top_image_embeddings(self, current_topic_id, training_doc_topic_distributions, top_n=10):
    ...
    def get_list_top_image_embeddings(self, training_doc_topic_distributions, top_n=10):
        list_top_image_embeddings = []
        for current_topic_id in range(self.n_components):
            list_top_image_embeddings.append(self.get_top_image_embeddings(current_topic_id, training_doc_topic_distributions, top_n))        
        return list_top_image_embeddings
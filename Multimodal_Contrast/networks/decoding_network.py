...
from Multimodal_Contrast.networks.inference_network import CombinedInferenceNetwork, ContextualInferenceNetwork
...
class ContrastiveM3LDecoderNetwork(nn.Module):
    ...
    def __init__(self, input_size, bert_sizes=(768, 2048), n_components=10, model_type='prodLDA',
        ...
        self.num_languages = 1#num_languages
        # each language has their own inference network (assume num_lang=2 for now) --- instantiate separate inference networks for each language/modality
        ...
        self.beta = torch.Tensor(self.num_languages, n_components, input_size)
        if torch.cuda.is_available():
    ...
    def forward(self, x_bow, x_bert, x_image):
        ...
        posterior_mu3, posterior_log_sigma3 = self.inf_net3(x_bow[:, 0, :], x_image)
    ...
    def get_theta(self, x, x_bert, lang_index=0, x_image=None):
        #x is X_bow
        with torch.no_grad():
            ...
            if lang_index == 0:
                ...
                posterior_mu, posterior_log_sigma = self.inf_net1(x[:, 0, :], x_bert[:, 0, :])
            # inference with language2
            #elif lang_index == 1:
                #posterior_mu, posterior_log_sigma = self.inf_net2(x, x_bert)
                #posterior_mu, posterior_log_sigma =  self.inf_net2(x[:, 1, :], x_bert[:, 1, :])
            # inference with image
            else:
                #posterior_mu, posterior_log_sigma = self.inf_net3(x, x_bert)
                posterior_mu, posterior_log_sigma =  self.inf_net3(x[:, 0, :], x_image)
